...
def main
  ...
  for num_1 in data do
    for num_2 in data do
      next if num_1 == num_2
      sum = num_1.dup.concat(num_2).map { |elem| { value: elem[:value], depth: elem[:depth] + 1 } }
      magnitude = [magnitude, calculate_magnitude(reduce(sum))].max 
    end
  end
